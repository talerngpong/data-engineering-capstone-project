...
def prepare_iam_role_arn(iam_client: IAMClient, iam_role_name: str) -> str:
    try:
        ...
        print(
            f'Role with name {iam_role_name} already exists. Continue next steps with expected exception = {e}')
...
def spin_up_and_wait_redshift_cluster(
    ...
    def get_available_cluster_props() -> ClusterTypeDef:
        ...
        if cluster_status.lower() == 'available':
            ...
            print(
                f"Redshift cluster with identifier = {identifier} is {cluster_status}. Let's wait for 5 seconds.")
            time.sleep(5)
...
def main() -> None:
    ...
    end_epoch = time.time()
    print(
        f'Redshift cluster preparation took {int(end_epoch - start_epoch)} seconds.')
    ...
    print(
        f'Successfully create cluster with metadata = {redshift_cluster_metadata}')
